...
/**
 * Analyse les produits du groupe 4 qui ont 'eggyolk' dans nova_groups_markers
 * Permet de comprendre pourquoi eggyolk est classé comme marqueur du groupe 4
 */
public class AnalyzeEggYolkInGroup4 {
    ...
    private static class ProductWithEggYolk {
        String code;
        String productName;
        int novaGroup;
        List<String> eggyolkMarkerTypes;  // Types où eggyolk apparaît (categories, ingredients, additives)
        List<String> allGroup4Markers;  // Tous les marqueurs du groupe 4
        
        ProductWithEggYolk(String code, String productName, int novaGroup) {
            this.code = code;
            this.productName = productName;
            this.novaGroup = novaGroup;
            this.eggyolkMarkerTypes = new ArrayList<>();
            this.allGroup4Markers = new ArrayList<>();
        }
    }
    
    private static List<ProductWithEggYolk> productsWithEggYolk = new ArrayList<>();

    public static void main(String[] args) {
        new NatclinnConf();
        try {
            System.out.println("=== Analyse des produits groupe 4 avec 'eggyolk' dans nova_groups_markers ===\n");
            ...
            analyzeEggYolkInGroup4();
            
            System.out.println("\n=== Produits trouvés avec eggyolk en groupe 4: " + productsWithEggYolk.size() + " ===\n");
            
            // Générer le fichier Excel
            String outputPath = NatclinnConf.folderForData + "/EggYolk_in_Group4_Analysis.xlsx";
    ...
    /**
     * Télécharge et analyse le fichier JSONL pour trouver les produits groupe 4 avec eggyolk
     */
    private static void analyzeEggYolkInGroup4() throws Exception {
        ...
        try (InputStream in = conn.getInputStream();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // Chercher les produits groupe 4 avec eggyolk
                    if (product.has("nova_group") && product.getInt("nova_group") == 4 && 
                        ...
                        if (markersObj.has("4")) {
                            ...
                            // Chercher eggyolk dans ce groupe
                            boolean hasEggYolk = false;
                            List<String> eggyolkTypes = new ArrayList<>();
                            ...
                            for (int i = 0; i < group4Markers.length(); i++) {
                                try {
                                    ...
                                    if (markerPair.length() >= 2) {
                                        ...
                                        if ("en:egg-yolk".equals(value)) {
                                            hasEggYolk = true;
                                            eggyolkTypes.add(type);
                                        }
                                    }
                                } catch (Exception e) {
                            ...
                            if (hasEggYolk) {
                                ...
                                ProductWithEggYolk p = new ProductWithEggYolk(code, productName, 4);
                                p.eggyolkMarkerTypes = eggyolkTypes;
                                p.allGroup4Markers = allMarkersInGroup4;
                                productsWithEggYolk.add(p);
                            }
                        }
                    }
        ...
        System.out.println(String.format("  Produits groupe 4 avec eggyolk: %,d", matchingProducts));
    }
    
    private static void generateExcelFile(String outputPath) throws Exception {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("EggYolk in Group 4");
        ...
        String[] headers = {"Code OFF", "Nom Produit", "EggYolk Types", "Tous les marqueurs groupe 4"};
        ...
        // Données
        int rowNum = 1;
        for (ProductWithEggYolk p : productsWithEggYolk) {
            ...
            cell2.setCellValue(String.join(", ", p.eggyolkMarkerTypes));
    ...
    private static void displaySummary() {
        System.out.println("\n=== Aperçu des produits avec eggyolk en groupe 4 ===");
        int count = 0;
        for (ProductWithEggYolk p : productsWithEggYolk) {
            if (count++ < 10) {
                System.out.println("\nCode: " + p.code);
                System.out.println("  Produit: " + p.productName);
                System.out.println("  EggYolk types: " + String.join(", ", p.eggyolkMarkerTypes));
        ...
        if (productsWithEggYolk.size() > 10) {
            System.out.println("\n... et " + (productsWithEggYolk.size() - 10) + " autres produits");
        }
        System.out.println("...");
    }
}
