...
/**
 * Analyse les produits du groupe 4 qui ont 'pepper' dans nova_groups_markers
 * Permet de comprendre pourquoi pepper est classé comme marqueur du groupe 4
 */
public class AnalyzePepperInGroup4 {
    ...
    private static class ProductWithPepper {
        String code;
        String productName;
        int novaGroup;
        List<String> pepperMarkerTypes;  // Types où pepper apparaît (categories, ingredients, additives)
        List<String> allGroup4Markers;  // Tous les marqueurs du groupe 4
        
        ProductWithPepper(String code, String productName, int novaGroup) {
            this.code = code;
            this.productName = productName;
            this.novaGroup = novaGroup;
            this.pepperMarkerTypes = new ArrayList<>();
            this.allGroup4Markers = new ArrayList<>();
        }
    }
    
    private static List<ProductWithPepper> productsWithPepper = new ArrayList<>();

    public static void main(String[] args) {
        new NatclinnConf();
        try {
            System.out.println("=== Analyse des produits groupe 4 avec 'pepper' dans nova_groups_markers ===\n");
            ...
            analyzePepperInGroup4();
            
            System.out.println("\n=== Produits trouvés avec pepper en groupe 4: " + productsWithPepper.size() + " ===\n");
            
            // Générer le fichier Excel
            String outputPath = NatclinnConf.folderForData + "/Pepper_in_Group4_Analysis.xlsx";
    ...
    /**
     * Télécharge et analyse le fichier JSONL pour trouver les produits groupe 4 avec pepper
     */
    private static void analyzePepperInGroup4() throws Exception {
        ...
        try (InputStream in = conn.getInputStream();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // Chercher les produits groupe 4 avec pepper
                    if (product.has("nova_group") && product.getInt("nova_group") == 4 && 
                        ...
                        if (markersObj.has("4")) {
                            ...
                            // Chercher pepper dans ce groupe
                            boolean hasPepper = false;
                            List<String> pepperTypes = new ArrayList<>();
                            ...
                            for (int i = 0; i < group4Markers.length(); i++) {
                                try {
                                    ...
                                    if (markerPair.length() >= 2) {
                                        ...
                                        if ("en:pepper".equals(value)) {
                                            hasPepper = true;
                                            pepperTypes.add(type);
                                        }
                                    }
                                } catch (Exception e) {
                            ...
                            if (hasPepper) {
                                ...
                                ProductWithPepper p = new ProductWithPepper(code, productName, 4);
                                p.pepperMarkerTypes = pepperTypes;
                                p.allGroup4Markers = allMarkersInGroup4;
                                productsWithPepper.add(p);
                            }
                        }
                    }
        ...
        System.out.println(String.format("  Produits groupe 4 avec pepper: %,d", matchingProducts));
    }
    
    private static void generateExcelFile(String outputPath) throws Exception {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Pepper in Group 4");
        ...
        String[] headers = {"Code OFF", "Nom Produit", "Pepper Types", "Tous les marqueurs groupe 4"};
        ...
        // Données
        int rowNum = 1;
        for (ProductWithPepper p : productsWithPepper) {
            ...
            cell2.setCellValue(String.join(", ", p.pepperMarkerTypes));
    ...
    private static void displaySummary() {
        ...
        for (ProductWithPepper p : productsWithPepper) {
            if (count++ < 10) {
                ...
                System.out.println("  Pepper types: " + String.join(", ", p.pepperMarkerTypes));
        ...
        if (productsWithPepper.size() > 10) {
            System.out.println("\n... et " + (productsWithPepper.size() - 10) + " autres produits");
        }
        System.out.println("...");
    }
}
