...
/**
 * Analyse les produits du groupe 4 qui ont 'potatostarch' dans nova_groups_markers
 * Permet de comprendre pourquoi potatostarch est classé comme marqueur du groupe 4
 */
public class AnalyzePotatoStarchInGroup4 {
    ...
    private static class ProductWithPotatoStarch {
        String code;
        String productName;
        int novaGroup;
        List<String> potatostarchMarkerTypes;  // Types où potatostarch apparaît (categories, ingredients, additives)
        List<String> allGroup4Markers;  // Tous les marqueurs du groupe 4
        
        ProductWithPotatoStarch(String code, String productName, int novaGroup) {
            this.code = code;
            this.productName = productName;
            this.novaGroup = novaGroup;
            this.potatostarchMarkerTypes = new ArrayList<>();
            this.allGroup4Markers = new ArrayList<>();
        }
    }
    
    private static List<ProductWithPotatoStarch> productsWithPotatoStarch = new ArrayList<>();

    public static void main(String[] args) {
        new NatclinnConf();
        try {
            System.out.println("=== Analyse des produits groupe 4 avec 'potatostarch' dans nova_groups_markers ===\n");
            ...
            analyzePotatoStarchInGroup4();
            
            System.out.println("\n=== Produits trouvés avec potatostarch en groupe 4: " + productsWithPotatoStarch.size() + " ===\n");
            
            // Générer le fichier Excel
            String outputPath = NatclinnConf.folderForData + "/PotatoStarch_in_Group4_Analysis.xlsx";
    ...
    /**
     * Télécharge et analyse le fichier JSONL pour trouver les produits groupe 4 avec potatostarch
     */
    private static void analyzePotatoStarchInGroup4() throws Exception {
        ...
        try (InputStream in = conn.getInputStream();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // Chercher les produits groupe 4 avec potatostarch
                    if (product.has("nova_group") && product.getInt("nova_group") == 4 && 
                        ...
                        if (markersObj.has("4")) {
                            ...
                            // Chercher potatostarch dans ce groupe
                            boolean hasPotatoStarch = false;
                            List<String> potatostarchTypes = new ArrayList<>();
                            ...
                            for (int i = 0; i < group4Markers.length(); i++) {
                                try {
                                    ...
                                    if (markerPair.length() >= 2) {
                                        ...
                                        if ("en:potato-starch".equals(value)) {
                                            hasPotatoStarch = true;
                                            potatostarchTypes.add(type);
                                        }
                                    }
                                } catch (Exception e) {
                            ...
                            if (hasPotatoStarch) {
                                ...
                                ProductWithPotatoStarch p = new ProductWithPotatoStarch(code, productName, 4);
                                p.potatostarchMarkerTypes = potatostarchTypes;
                                p.allGroup4Markers = allMarkersInGroup4;
                                productsWithPotatoStarch.add(p);
                            }
                        }
                    }
        ...
        System.out.println(String.format("  Produits groupe 4 avec potatostarch: %,d", matchingProducts));
    }
    
    private static void generateExcelFile(String outputPath) throws Exception {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("PotatoStarch in Group 4");
        ...
        String[] headers = {"Code OFF", "Nom Produit", "PotatoStarch Types", "Tous les marqueurs groupe 4"};
        ...
        // Données
        int rowNum = 1;
        for (ProductWithPotatoStarch p : productsWithPotatoStarch) {
            ...
            cell2.setCellValue(String.join(", ", p.potatostarchMarkerTypes));
    ...
    private static void displaySummary() {
        ...
        for (ProductWithPotatoStarch p : productsWithPotatoStarch) {
            if (count++ < 10) {
                ...
                System.out.println("  PotatoStarch types: " + String.join(", ", p.potatostarchMarkerTypes));
        ...
        if (productsWithPotatoStarch.size() > 10) {
            System.out.println("\n... et " + (productsWithPotatoStarch.size() - 10) + " autres produits");
        }
        System.out.println("...");
    }
}
