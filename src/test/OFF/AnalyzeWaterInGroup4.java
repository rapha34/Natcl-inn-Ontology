...
/**
 * Analyse les produits du groupe 4 qui ont 'water' dans nova_groups_markers
 * Permet de comprendre pourquoi water est classé comme marqueur du groupe 4
 */
public class AnalyzeWaterInGroup4 {
    ...
    private static class ProductWithWater {
        String code;
        String productName;
        int novaGroup;
        List<String> waterMarkerTypes;  // Types où water apparaît (categories, ingredients, additives)
        List<String> allGroup4Markers;  // Tous les marqueurs du groupe 4
        
        ProductWithWater(String code, String productName, int novaGroup) {
            this.code = code;
            this.productName = productName;
            this.novaGroup = novaGroup;
            this.waterMarkerTypes = new ArrayList<>();
            this.allGroup4Markers = new ArrayList<>();
        }
    }
    
    private static List<ProductWithWater> productsWithWater = new ArrayList<>();

    public static void main(String[] args) {
        new NatclinnConf();
        try {
            System.out.println("=== Analyse des produits groupe 4 avec 'water' dans nova_groups_markers ===\n");
            ...
            analyzeWaterInGroup4();
            
            System.out.println("\n=== Produits trouvés avec water en groupe 4: " + productsWithWater.size() + " ===\n");
            
            // Générer le fichier Excel
            String outputPath = NatclinnConf.folderForData + "/Water_in_Group4_Analysis.xlsx";
    ...
    /**
     * Télécharge et analyse le fichier JSONL pour trouver les produits groupe 4 avec water
     */
    private static void analyzeWaterInGroup4() throws Exception {
        ...
        try (InputStream in = conn.getInputStream();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // Chercher les produits groupe 4 avec water
                    if (product.has("nova_group") && product.getInt("nova_group") == 4 && 
                        ...
                        if (markersObj.has("4")) {
                            ...
                            if (hasWater) {
                                ...
                                ProductWithWater p = new ProductWithWater(code, productName, 4);
                                p.waterMarkerTypes = waterTypes;
                                p.allGroup4Markers = allMarkersInGroup4;
                                productsWithWater.add(p);
                            }
                        }
                    }
        ...
        System.out.println(String.format("  Produits groupe 4 avec water: %,d", matchingProducts));
    }
    
    private static void generateExcelFile(String outputPath) throws Exception {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Water in Group 4");
        ...
        String[] headers = {"Code OFF", "Nom Produit", "Water Types", "Tous les marqueurs groupe 4"};
        ...
        // Données
        int rowNum = 1;
        for (ProductWithWater p : productsWithWater) {
            ...
            cell2.setCellValue(String.join(", ", p.waterMarkerTypes));
    ...
    private static void displaySummary() {
        ...
        for (ProductWithWater p : productsWithWater) {
            if (count++ < 10) {
                ...
                System.out.println("  Water types: " + String.join(", ", p.waterMarkerTypes));
        ...
        if (productsWithWater.size() > 10) {
            System.out.println("\n... et " + (productsWithWater.size() - 10) + " autres produits");
        }
        System.out.println("...");
    }
}
