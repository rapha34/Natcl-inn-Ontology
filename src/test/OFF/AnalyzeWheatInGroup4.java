...
/**
 * Analyse les produits du groupe 4 qui ont 'wheat' dans nova_groups_markers
 * Permet de comprendre pourquoi wheat est classé comme marqueur du groupe 4
 */
public class AnalyzeWheatInGroup4 {
    ...
    private static class ProductWithWheat {
        String code;
        String productName;
        int novaGroup;
        List<String> wheatMarkerTypes;  // Types où wheat apparaît (categories, ingredients, additives)
        List<String> allGroup4Markers;  // Tous les marqueurs du groupe 4
        
        ProductWithWheat(String code, String productName, int novaGroup) {
            this.code = code;
            this.productName = productName;
            this.novaGroup = novaGroup;
            this.wheatMarkerTypes = new ArrayList<>();
            this.allGroup4Markers = new ArrayList<>();
        }
    }
    
    private static List<ProductWithWheat> productsWithWheat = new ArrayList<>();

    public static void main(String[] args) {
        new NatclinnConf();
        try {
            System.out.println("=== Analyse des produits groupe 4 avec 'wheat' dans nova_groups_markers ===\n");
            ...
            analyzeWheatInGroup4();
            
            System.out.println("\n=== Produits trouvés avec wheat en groupe 4: " + productsWithWheat.size() + " ===\n");
            
            // Générer le fichier Excel
            String outputPath = NatclinnConf.folderForData + "/Wheat_in_Group4_Analysis.xlsx";
    ...
    /**
     * Télécharge et analyse le fichier JSONL pour trouver les produits groupe 4 avec wheat
     */
    private static void analyzeWheatInGroup4() throws Exception {
        ...
        try (InputStream in = conn.getInputStream();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // Chercher les produits groupe 4 avec wheat
                    if (product.has("nova_group") && product.getInt("nova_group") == 4 && 
                        ...
                        if (markersObj.has("4")) {
                            ...
                            // Chercher wheat dans ce groupe
                            boolean hasWheat = false;
                            List<String> wheatTypes = new ArrayList<>();
                            ...
                            if (hasWheat) {
                                ...
                                ProductWithWheat p = new ProductWithWheat(code, productName, 4);
                                p.wheatMarkerTypes = wheatTypes;
                                p.allGroup4Markers = allMarkersInGroup4;
                                productsWithWheat.add(p);
                            }
                        }
                    }
        ...
        System.out.println(String.format("  Produits groupe 4 avec wheat: %,d", matchingProducts));
    }
    
    private static void generateExcelFile(String outputPath) throws Exception {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Wheat in Group 4");
        ...
        String[] headers = {"Code OFF", "Nom Produit", "Wheat Types", "Tous les marqueurs groupe 4"};
        ...
        // Données
        int rowNum = 1;
        for (ProductWithWheat p : productsWithWheat) {
            ...
            cell2.setCellValue(String.join(", ", p.wheatMarkerTypes));
    ...
    private static void displaySummary() {
        ...
        for (ProductWithWheat p : productsWithWheat) {
            if (count++ < 10) {
                ...
                System.out.println("  Wheat types: " + String.join(", ", p.wheatMarkerTypes));
        ...
        if (productsWithWheat.size() > 10) {
            System.out.println("\n... et " + (productsWithWheat.size() - 10) + " autres produits");
        }
        System.out.println("...");
    }
}
