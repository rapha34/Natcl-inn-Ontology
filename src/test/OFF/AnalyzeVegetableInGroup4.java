...
/**
 * Analyse les produits du groupe 4 qui ont 'vegetable' dans nova_groups_markers
 * Permet de comprendre pourquoi vegetable est classé comme marqueur du groupe 4
 */
public class AnalyzeVegetableInGroup4 {
    ...
    private static class ProductWithVegetable {
        String code;
        String productName;
        int novaGroup;
        List<String> vegetableMarkerTypes;  // Types où vegetable apparaît (categories, ingredients, additives)
        List<String> allGroup4Markers;  // Tous les marqueurs du groupe 4
        
        ProductWithVegetable(String code, String productName, int novaGroup) {
            this.code = code;
            this.productName = productName;
            this.novaGroup = novaGroup;
            this.vegetableMarkerTypes = new ArrayList<>();
            this.allGroup4Markers = new ArrayList<>();
        }
    }
    
    private static List<ProductWithVegetable> productsWithVegetable = new ArrayList<>();

    public static void main(String[] args) {
        new NatclinnConf();
        try {
            System.out.println("=== Analyse des produits groupe 4 avec 'vegetable' dans nova_groups_markers ===\n");
            ...
            analyzeVegetableInGroup4();
            
            System.out.println("\n=== Produits trouvés avec vegetable en groupe 4: " + productsWithVegetable.size() + " ===\n");
            
            // Générer le fichier Excel
            String outputPath = NatclinnConf.folderForData + "/Vegetable_in_Group4_Analysis.xlsx";
    ...
    /**
     * Télécharge et analyse le fichier JSONL pour trouver les produits groupe 4 avec vegetable
     */
    private static void analyzeVegetableInGroup4() throws Exception {
        ...
        try (InputStream in = conn.getInputStream();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // Chercher les produits groupe 4 avec vegetable
                    if (product.has("nova_group") && product.getInt("nova_group") == 4 && 
                        ...
                        if (markersObj.has("4")) {
                            ...
                            // Chercher vegetable dans ce groupe
                            boolean hasVegetable = false;
                            List<String> vegetableTypes = new ArrayList<>();
                            ...
                            if (hasVegetable) {
                                ...
                                ProductWithVegetable p = new ProductWithVegetable(code, productName, 4);
                                p.vegetableMarkerTypes = vegetableTypes;
                                p.allGroup4Markers = allMarkersInGroup4;
                                productsWithVegetable.add(p);
                            }
                        }
                    }
        ...
        System.out.println(String.format("  Produits groupe 4 avec vegetable: %,d", matchingProducts));
    }
    
    private static void generateExcelFile(String outputPath) throws Exception {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Vegetable in Group 4");
        ...
        String[] headers = {"Code OFF", "Nom Produit", "Vegetable Types", "Tous les marqueurs groupe 4"};
        ...
        // Données
        int rowNum = 1;
        for (ProductWithVegetable p : productsWithVegetable) {
            ...
            cell2.setCellValue(String.join(", ", p.vegetableMarkerTypes));
    ...
    private static void displaySummary() {
        System.out.println("\n=== Aperçu des produits avec vegetable en groupe 4 ===");
        int count = 0;
        for (ProductWithVegetable p : productsWithVegetable) {
            if (count++ < 10) {
                System.out.println("\nCode: " + p.code);
                System.out.println("  Produit: " + p.productName);
                System.out.println("  Vegetable types: " + String.join(", ", p.vegetableMarkerTypes));
        ...
        if (productsWithVegetable.size() > 10) {
            System.out.println("\n... et " + (productsWithVegetable.size() - 10) + " autres produits");
        }
        System.out.println("...");
    }
}
