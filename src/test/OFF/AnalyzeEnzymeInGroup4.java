...
/**
 * Analyse les produits du groupe 4 qui ont 'enzyme' dans nova_groups_markers
 * Permet de comprendre pourquoi enzyme est classé comme marqueur du groupe 4
 */
public class AnalyzeEnzymeInGroup4 {
    ...
    private static class ProductWithEnzyme {
        String code;
        String productName;
        int novaGroup;
        List<String> enzymeMarkerTypes;  // Types où enzyme apparaît (categories, ingredients, additives)
        List<String> allGroup4Markers;  // Tous les marqueurs du groupe 4
        
        ProductWithEnzyme(String code, String productName, int novaGroup) {
            this.code = code;
            this.productName = productName;
            this.novaGroup = novaGroup;
            this.enzymeMarkerTypes = new ArrayList<>();
            this.allGroup4Markers = new ArrayList<>();
        }
    }
    
    private static List<ProductWithEnzyme> productsWithEnzyme = new ArrayList<>();

    public static void main(String[] args) {
        new NatclinnConf();
        try {
            System.out.println("=== Analyse des produits groupe 4 avec 'enzyme' dans nova_groups_markers ===\n");
            ...
            analyzeEnzymeInGroup4();
            
            System.out.println("\n=== Produits trouvés avec enzyme en groupe 4: " + productsWithEnzyme.size() + " ===\n");
            
            // Générer le fichier Excel
            String outputPath = NatclinnConf.folderForData + "/Enzyme_in_Group4_Analysis.xlsx";
    ...
    /**
     * Télécharge et analyse le fichier JSONL pour trouver les produits groupe 4 avec enzyme
     */
    private static void analyzeEnzymeInGroup4() throws Exception {
        ...
        try (InputStream in = conn.getInputStream();
            ...
            while ((line = reader.readLine()) != null) {
                ...
                try {
                    ...
                    // Chercher les produits groupe 4 avec enzyme
                    if (product.has("nova_group") && product.getInt("nova_group") == 4 && 
                        ...
                        if (markersObj.has("4")) {
                            ...
                            // Chercher enzyme dans ce groupe
                            boolean hasEnzyme = false;
                            List<String> enzymeTypes = new ArrayList<>();
                            ...
                            for (int i = 0; i < group4Markers.length(); i++) {
                                try {
                                    ...
                                    if (markerPair.length() >= 2) {
                                        ...
                                        if ("en:enzyme".equals(value)) {
                                            hasEnzyme = true;
                                            enzymeTypes.add(type);
                                        }
                                    }
                                } catch (Exception e) {
                            ...
                            if (hasEnzyme) {
                                ...
                                ProductWithEnzyme p = new ProductWithEnzyme(code, productName, 4);
                                p.enzymeMarkerTypes = enzymeTypes;
                                p.allGroup4Markers = allMarkersInGroup4;
                                productsWithEnzyme.add(p);
                            }
                        }
                    }
        ...
        System.out.println(String.format("  Produits groupe 4 avec enzyme: %,d", matchingProducts));
    }
    
    private static void generateExcelFile(String outputPath) throws Exception {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Enzyme in Group 4");
        ...
        String[] headers = {"Code OFF", "Nom Produit", "Enzyme Types", "Tous les marqueurs groupe 4"};
        ...
        // Données
        int rowNum = 1;
        for (ProductWithEnzyme p : productsWithEnzyme) {
            ...
            cell2.setCellValue(String.join(", ", p.enzymeMarkerTypes));
    ...
    private static void displaySummary() {
        System.out.println("\n=== Aperçu des produits avec enzyme en groupe 4 ===");
        int count = 0;
        for (ProductWithEnzyme p : productsWithEnzyme) {
            if (count++ < 10) {
                System.out.println("\nCode: " + p.code);
                System.out.println("  Produit: " + p.productName);
                System.out.println("  Enzyme types: " + String.join(", ", p.enzymeMarkerTypes));
        ...
        if (productsWithEnzyme.size() > 10) {
            System.out.println("\n... et " + (productsWithEnzyme.size() - 10) + " autres produits");
        }
        System.out.println("...");
    }
}
