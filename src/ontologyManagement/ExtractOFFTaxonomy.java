...
/**
 * Extraction de la taxonomie des ingrédients depuis Open Food Facts
 * Télécharge le fichier JSON et génère un fichier Excel structuré
 */
public class ExtractOFFTaxonomy {

    private static final String OFF_TAXONOMY_URL = "https://world.openfoodfacts.org/data/taxonomies/ingredients.json";
    ...
    public static void main(String[] args) {
        new NatclinnConf();
        try {
            System.out.println("=== Extraction de la taxonomie OFF (ingrédients) ===");
            ...
            System.out.println("\n=== Total d'ingrédients dans la taxonomie: " + taxonomy.size() + " ===\n");
            
            // Générer le fichier Excel
            String outputPath = NatclinnConf.folderForData + "/OFF_taxonomy_ingredients.xlsx";
    ...
    private static void downloadAndParseTaxonomy() throws Exception {
        ...
        int processed = 0;
        for (String ingredientId : taxonomyJson.keySet()) {
            processed++;
            
            if (processed % 1000 == 0) {
                System.out.println("  Traité: " + processed + " ingrédients...");
            }
            
            try {
                JSONObject ingredientData = taxonomyJson.getJSONObject(ingredientId);
                TaxonomyElement element = new TaxonomyElement(ingredientId);
                
                // Extraire les noms
                if (ingredientData.has("name")) {
                    JSONObject names = ingredientData.getJSONObject("name");
                    if (names.has("en")) {
                ...
                if (ingredientData.has("parents")) {
                    JSONArray parents = ingredientData.getJSONArray("parents");
                ...
                if (ingredientData.has("children")) {
                    JSONArray children = ingredientData.getJSONArray("children");
                    for (int i = 0; i < children.length(); i++) {
                        element.children.add(children.getString(i));
                    }
                }
                
                taxonomy.put(ingredientId, element);
                
            } catch (Exception e) {
                System.err.println("  ⚠ Erreur pour l'ingrédient: " + ingredientId);
            }
        }
        
        long elapsed = (System.currentTimeMillis() - startTime) / 1000;
        System.out.println("Parsing terminé en " + elapsed + " secondes");
        System.out.println("  Total ingrédients traités: " + processed);
    }
    ...
    private static void generateExcelFile(String outputPath) throws Exception {
        ...
        // Feuille 1: Liste complète des ingrédients
        Sheet ingredientsSheet = workbook.createSheet("Ingredients");
        createIngredientsSheet(ingredientsSheet, workbook);
    ...
    /**
     * Crée la feuille avec la liste des ingrédients
     */
    private static void createIngredientsSheet(Sheet sheet, Workbook workbook) {
        // Styles
    ...
    private static void createHierarchySheet(Sheet sheet, Workbook workbook) {
        ...
        // En-têtes
        Row headerRow = sheet.createRow(0);
        String[] headers = {"Ingrédient", "Nom EN", "Parent", "Nom Parent EN", "Niveau"};
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            cell.setCellStyle(headerStyle);
        ...
        // Trier par ingrédient puis parent
        relations.sort((a, b) -> {
    ...
    private static void createStatsSheet(Sheet sheet, Workbook workbook) {
        ...
        addStatRow(sheet, rowNum++, "Total ingrédients", taxonomy.size(), headerStyle, dataStyle);
        ...
        // Top 10 des ingrédients avec le plus d'enfants
        ...
        createCell(headerRow, 0, "Ingrédient", headerStyle);
    ...
    /**
     * Calcule le niveau hiérarchique d'un ingrédient (profondeur depuis la racine)
     */
    private static int calculateLevel(String ingredientId) {
        TaxonomyElement element = taxonomy.get(ingredientId);
    ...
    private static void displaySummary() {
        ...
        System.out.println("Total ingrédients: " + taxonomy.size());
        ...
        // Chercher "en:emmental"
        if (taxonomy.containsKey("en:emmental")) {
            System.out.println("\nExemple: en:emmental");
            displayHierarchy("en:emmental", 0);
        }
        
        // Chercher "en:sunflower-oil"
        if (taxonomy.containsKey("en:sunflower-oil")) {
            System.out.println("\nExemple: en:sunflower-oil");
            displayHierarchy("en:sunflower-oil", 0);
        }
        
        // Chercher "en:water"
        if (taxonomy.containsKey("en:water")) {
            System.out.println("\nExemple: en:water");
            displayHierarchy("en:water", 0);
        }
    }
    
    /**
     * Affiche la hiérarchie d'un ingrédient
     */
    private static void displayHierarchy(String ingredientId, int level) {
        TaxonomyElement element = taxonomy.get(ingredientId);
        ...
        System.out.println(indent + ingredientId + " (" + name + ")");
