...
public class CalcRVarVector extends BaseBuiltin {

	@Override
	public String getName() {
		return "calcRVarVector";
	}
		...
		if (javaVector.isLiteral()) {
			...
			if (javaVector.getLiteralValue() instanceof String) {
				if (!javaVector.getLiteralValue().equals(""))  {
					...
					double nVariance = RUtil.Var(strJavaVector);
					//System.out.println(nVariance);
					// Creating a node for the output parameter
					variance = Util.makeDoubleNode(nVariance);
					//System.out.println(variance);
					// Binding the output parameter to the node
					BindingEnvironment env = context.getEnv();
					success = env.bind(args[1], variance);
				}
			}
		} 
		return success;
