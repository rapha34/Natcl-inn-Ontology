...
public class CalcScore extends BaseBuiltin {
    ...
    if (number1.isLiteral() && number2.isLiteral() ) {
    	...
    	if (number1.getLiteralValue() instanceof Number && 
    		...
    		if (nvNumber2.doubleValue() > 0.0) {
    			double nScore = (nvNumber1.doubleValue() + nvNumber2.doubleValue())/2;
    			// Creating a node for the output parameter
    			score = Util.makeDoubleNode(nScore);
    		}
    		// Binding the output parameter to the node
    		BindingEnvironment env = context.getEnv();
    		success = env.bind(args[2], score);
    	} 
    }   
    return success;
  }
