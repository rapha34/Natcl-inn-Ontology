...
public class CalcRSumVector extends BaseBuiltin {

	@Override
	public String getName() {
		return "calcRSumVector";
	}
		...
		if (javaVector.isLiteral()) {
			...
			if (javaVector.getLiteralValue() instanceof String) {
				if (!javaVector.getLiteralValue().equals(""))  {
					...
					double nSum = RUtil.Sum(strJavaVector);

					// Creating a node for the output parameter
					sum = Util.makeDoubleNode(nSum);
					//System.out.println(sum);
					// Binding the output parameter to the node
					BindingEnvironment env = context.getEnv();
					success = env.bind(args[1], sum);
				}
			}
		} 
		return success;
